...
function Filtra (src) {
  if (!src) throw new FiltraError(this.key, 'empty body')
  return new FiltraChainLink(Object.assign({}, src), {})
}

function FiltraError (param, message) {
  this.name = 'FiltraError'
  this.message = (message || '')
  const error = new Error(this.message)
  error.name = this.name
  this.stack = error.stack
  this.status = 400 // bad request cheat
  this.param = param
}
...
class FiltraChainLink {
  ...
  require (key) {
    if (typeof this.src[key] === 'undefined' || this.src[key] === null) throw new FiltraError(this.key, `${key} required`)
    return this
  }
  delete (key) {
    delete this.dst[key]
...
class Option extends FiltraChainLink {
  ...
  check (predicate, message) {
    if (!predicate(this.get())) {
      const m = message || `${this.key} is incorrect`
      throw new FiltraError(this.key, m)
    }
    return this
  }
  any (allowable) {
    const x = this.dst[this.key]
    if (x === null) return this
    if (!allowable.includes(x)) throw new FiltraError(this.key, `${this.key} not allowed`)
    return this
  }
  in (allowable) { return this.any(allowable) }
  require () {
    if (this.dst[this.key] === null) throw new FiltraError(this.key, `${this.key} required`)
    return this
  }
  rename (newKey) {
  ...
  eq (val) {
    if (this.get() !== val) {
      throw new FiltraError(this.key, `${this.key} is incorrect`)
    }
    return this
  }
}
...
class FlagOption extends Option {
  constructor (src, dst, key, defaultValue) {
    ...
    case 'no':
      ...
      break
    default:
      throw new FiltraError(this.key, `${key} is not a boolean`)
    }
  }
}
  ...
  constructor (src, dst, key, defaultValue) {
    ...
    if (this.get() !== null) {
      if (typeof this.get() !== 'string') {
        throw new FiltraError(this.key, `${key} is not a string`)
      }
    }
  }
  notEmpty () {
    const x = this.dst[this.key]
    if (x === null) return this
    if (x.length <= 0) throw new FiltraError(this.key, `${this.key} is empty`)
    return this
  }
  match (regExp) {
    const x = this.dst[this.key]
    if (x === null) return this
    if (!regExp.test(x)) throw new FiltraError(this.key, `${this.key} is not match ${regExp.toString()}`)
    return this
  }
  notMatch (regExp) {
    const x = this.dst[this.key]
    if (x === null) return this
    if (regExp.test(x)) throw new FiltraError(this.key, `${this.key} is match ${regExp.toString()}`)
    return this
  }
  min (minLength) {
    const x = this.dst[this.key]
    if (x === null) return this
    if (x.length < minLength) throw new FiltraError(this.key, `${this.key} length can't be less than ${minLength}`)
    return this
  }
  max (maxLength) {
    const x = this.dst[this.key]
    if (x === null) return this
    if (x.length > maxLength) throw new FiltraError(this.key, `${this.key} length can't be more than ${maxLength}`)
    return this
  }
  range (min, max) {
    const x = this.dst[this.key]
    ...
    if (x.length < min || x.length > max) {
      throw new FiltraError(this.key, `${this.key} length is not in the range[${min}, ${max}]`)
    }
    return this
  }
  toUpperCase () {
...
class NumOption extends Option {
  ...
  min (min) {
    const x = this.dst[this.key]
    if (x === null) return this
    if (x < min) throw new FiltraError(this.key, `${this.key} can't be less than ${min}`)
    return this
  }
  max (max) {
    const x = this.dst[this.key]
    if (x === null) return this
    if (x > max) throw new FiltraError(this.key, `${this.key} can't be more than ${max}`)
    return this
  }
  range (min, max) {
    const x = this.dst[this.key]
    if (x === null) return this
    if (x < min || x > max) {
      throw new FiltraError(this.key, `${this.key} is not in the range[${min}, ${max}]`)
    }
    return this
  }
  integer () {
...
class UuidV4Option extends Option {
  constructor (src, dst, key, defaultValue) {
    ...
    this.set(this.get() !== null ? this.get().toLowerCase() : null)
  }
}
...
class FormattedDateOption extends Option {
  format (format, flag) {
    ...
    if (x === null) return this
    this.format = format
    x = this.dst[this.key] = moment(x, format, flag || false)
    if (!x.isValid()) throw new FiltraError(this.key, `${this.key} is not match ${format}`)
    return this
  }
  min (min) {
    ...
    const x = this.dst[this.key]
    if (x === null) return this
    if (min.isAfter(x)) throw new FiltraError(this.key, `${this.key} can't be earlier than ${min}`)
    return this
  }
  max (max) {
    ...
    const x = this.dst[this.key]
    if (x === null) return this
    if (x.isAfter(max)) throw new FiltraError(this.key, `${this.key} can't be later than ${max}`)
    return this
  }
  range (min, max) {
    if (typeof min === 'string') {
